...
// The API key lives in Apps Script > Project Settings > Script Properties
// under the key RESEND_API_KEY (never commit it to this public repo).
...
function sendResendEmail(toEmail, toName, partnerName, partnerSlack) {
  if (!RESEND_API_KEY) {
    Logger.log('RESEND_API_KEY script property is not set - email to ' + toEmail + ' skipped');
...
function doPost(e) {
  ...
  if (data.action === 'runLottery') {
    ...
    if (!lock.tryLock(30000)) {
      return ContentService
        .createTextOutput(JSON.stringify({ success: false, error: 'Lottery is already running' }))
        .setMimeType(ContentService.MimeType.JSON);
    }
    try {
      return runLotteryLocked(ss);
    } finally {
      lock.releaseLock();
    }
  }
